...
import engine.models.cifar.vgg as vgg
...
def runner(args, req):
    ...
    model = torch.load('/root/data/workspace/prunemethod/resnet56/ours-v0/cifar10_resnet56_surrogate_step5.pth')
    model.eval()
    ...
    print("ops: {:.4f} M".format(base_ops / 1e6))
    print("="*16)
    ...
    print("Ops: {:.2f} M => {:.2f} M ({:.2f}%, {:.2f}X )".format(base_ops / 1e6, pruned_ops / 1e6, pruned_ops / base_ops * 100, base_ops / pruned_ops))
    print("="*16)
